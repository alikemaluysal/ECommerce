...
public class CategoriesController : BaseController
{
    ...
    [HttpPut("{id}")]
    public async Task<ActionResult<UpdatedCategoryResponse>> Update([FromRoute] Guid id, [FromBody] UpdateCategoryRequest request)
    {
    ...
    public async Task<ActionResult<List<GetTopCategoriesResponse>>> GetTopCategories()
    {
        GetTopCategoriesQuery getTopCategoriesQuery = new();
        List<GetTopCategoriesResponse> response = await Mediator.Send(getTopCategoriesQuery);
        return Ok(response);
    }
}
